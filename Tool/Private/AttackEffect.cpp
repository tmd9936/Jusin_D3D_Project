	...
	const _float& fX, const _float& fY, const _float& fZ, _bool collisionEffect)
{
	if (m_pColliderCom)
	{
		...
		if (m_pAttackCom)
		{
			...
			{
				if (false == pOtherHpCom->Get_CanGetDamage())
					return;
...
void CAttackEffect::Play_DamageSound()
{
	...
	case 0:
		...
		CGameInstance::GetInstance()->PlaySoundW(L"SE_Status_Damage_01_a01.ogg", 0.8f);
		break;
	case 1:
		//CGameInstance::GetInstance()->StopSound(SOUND_GET_DAMAGE_2);
		CGameInstance::GetInstance()->PlaySoundW(L"SE_Status_Damage_01_a02.ogg", 0.8f);
		break;
	case 2:
	...
	case 3:
		...
		CGameInstance::GetInstance()->PlaySoundW(L"SE_Status_Damage_01_a04.ogg", 0.8f);
		break;
	case 4:
		//CGameInstance::GetInstance()->StopSound(SOUND_GET_DAMAGE_5);
		CGameInstance::GetInstance()->PlaySoundW(L"SE_Status_Damage_01_a05.ogg", 0.8f);
		break;
	case 5:
		//CGameInstance::GetInstance()->StopSound(SOUND_GET_DAMAGE_6);
		CGameInstance::GetInstance()->PlaySoundW(L"SE_Status_Damage_01_a06.ogg", 0.8f);
		break;
	}
